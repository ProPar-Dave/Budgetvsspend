...
Correct KPI numeric formatting so values are displayed consistently and appropriately for the active metric mode.

This is a formatting fix only.
...
2. FIX PPD MODE KPI FORMATTING

When the report is in PPD mode:
- KPI values must not use a dollar sign
- KPI values must still use comma separators
...
3. FIX DOLLAR MODE KPI FORMATTING

When the report is in Dollar mode:
- KPI values must always include a dollar sign
- KPI values must always use comma separators
- KPI values must always display exactly 2 decimal places
...
- must not receive a dollar sign
- must not receive unnecessary decimal changes unless already required by the current format rules
- should remain percentage values

Example:
- -23% stays -23%

5. APPLY THE FIX ONLY TO KPI VALUES

This pass is for KPI formatting only.

Do not change table cell formatting unless it is already governed separately.
...
2. PPD MODE KPI VALUES NO LONGER USE DOLLAR SIGNS
3. DOLLAR MODE KPI VALUES NOW USE $ AND 2 DECIMAL PLACES
